defmodule Gothamcityhall.TimeTest do
  use Gothamcityhall.DataCase

  alias Gothamcityhall.Time

  describe "clocks" do
    alias Gothamcityhall.Time.Clock

    import Gothamcityhall.TimeFixtures
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~U[2022-10-24 08:54:00Z]}

      assert {:ok, %Clock{} = clock} = Time.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~U[2022-10-24 08:54:00Z]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~U[2022-10-25 08:54:00Z]}

      assert {:ok, %Clock{} = clock} = Time.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~U[2022-10-25 08:54:00Z]
    end
