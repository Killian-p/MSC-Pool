defmodule Gothamcity.WorkingtimesTest do
  use Gothamcity.DataCase

  alias Gothamcity.Workingtimes

  describe "workingtimes" do
    alias Gothamcity.Workingtimes.Workingtime

    import Gothamcity.WorkingtimesFixtures
    ...
    test "create_workingtime/1 with valid data creates a workingtime" do
      valid_attrs = %{end: ~U[2022-10-24 11:14:00Z], start: ~U[2022-10-24 11:14:00Z]}

      assert {:ok, %Workingtime{} = workingtime} = Workingtimes.create_workingtime(valid_attrs)
      assert workingtime.end == ~U[2022-10-24 11:14:00Z]
      assert workingtime.start == ~U[2022-10-24 11:14:00Z]
    end
    ...
    test "update_workingtime/2 with valid data updates the workingtime" do
      workingtime = workingtime_fixture()
      update_attrs = %{end: ~U[2022-10-25 11:14:00Z], start: ~U[2022-10-25 11:14:00Z]}

      assert {:ok, %Workingtime{} = workingtime} = Workingtimes.update_workingtime(workingtime, update_attrs)
      assert workingtime.end == ~U[2022-10-25 11:14:00Z]
      assert workingtime.start == ~U[2022-10-25 11:14:00Z]
    end
