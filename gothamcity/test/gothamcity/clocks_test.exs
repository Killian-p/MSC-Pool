defmodule Gothamcity.ClocksTest do
  use Gothamcity.DataCase

  alias Gothamcity.Clocks

  describe "clocks" do
    alias Gothamcity.Clocks.Clock

    import Gothamcity.ClocksFixtures
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~U[2022-10-24 11:13:00Z]}

      assert {:ok, %Clock{} = clock} = Clocks.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~U[2022-10-24 11:13:00Z]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~U[2022-10-25 11:13:00Z]}

      assert {:ok, %Clock{} = clock} = Clocks.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~U[2022-10-25 11:13:00Z]
    end
